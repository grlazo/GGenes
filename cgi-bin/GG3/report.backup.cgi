#!/usr/bin/perl -I/www/cgi-bin/GG3
...
#NLremoved to add link to comment.cgi: print $cgi->p({-style=>'font-size: 11px;'},'[ '.$cgi->a({-href=>&get_self_url($cgi,{'print'=>''}),-target=>'_blank'},'Printable Version').' ]') unless (defined($cgi->param('print')));
    print $cgi->p({-style=>'font-size: 11px;'},
    		   ...
    		   .'&nbsp;&nbsp;'
                   .'[ '.$cgi->a({-href=>"$cgiurlpath/comment.cgi?class=$class;name=".&geturlstring($name)."&sendto=curator\@pw.usda.gov&print=''&show=all",-target=>'_blank'},'Submit comment/correction').' ]') unless (defined($cgi->param('print')));
    ...
    print $cgi->start_table({-cellpadding=>0,-cellspacing=>0});
    print "\n\n\n";
...
sub print_element {
         ...
         ($label eq 'Probe' || $label eq 'Gene' || $label eq 'Locus'))
    {
      ...
      print $cgi->td({-style=>'vertical-align:top'},$cgi->b($label));
      print "\n";
    }
    else
    {
    ...
    #end replacement
    print $cgi->start_td({-style=>'vertical-align:top'});
    #//print $cgi->start_table({-cellpadding=>0,-cellspacing=>0});
	...
	foreach my $cell (@$cells) {
		...
		# complete reference
		#//print $cgi->td({-valign=>'top',-colspan=>scalar(@$cells)},&get_complete_reference($cgi,$dbh,$row->{'reference_id'}));
		print '<div>' . &get_complete_reference($cgi,$dbh,$row->{'reference_id'}) . "</div>\n";
	    } elsif ($cell eq 'url') {
...
#		    print '<div style="float:left;padding-right:5px;">';
		    print '<div>';
		    # change to name param DDH 050203
		    print $cgi->a({-href=>sprintf("$cgiurlpath/report.cgi?class=$class;name=%s",&geturlstring($row->{$name}))},
...
#		    print $cgi->td({-valign=>'top',-nowrap=>undef},$row->{$col});
		    print '<div>' . $row->{$col} . "</div>\n";
#		    print '<div style="float:left;padding-right:5px;">' . $row->{$col} . "</div>\n";
		} else {
		    # just print it
		    print '<div>' . $row->{$col} . "</div>\n";
#		    print '<div style="float:left;padding-right:5px;">' . $row->{$col} . "</div>\n";
		...
		if (!exists($row->{$cell}) || !defined($row->{$cell}) || ($row->{$cell} =~ /^\s*$/)) {
		    ...
		    # nowrap
#//		    print $cgi->td({-valign=>'topaaa',-nowrap=>undef},$cgi->escapeHTML($row->{$cell}));
#		    print "<div style=\"float:left;padding-right:5px;\">" . $cgi->escapeHTML($row->{$cell}) . "</div>\n";
		    print "<div>" . $cgi->escapeHTML($row->{$cell}) . "</div>\n";
		} else {
		    # just print it
#		    print $cgi->td({-valign=>'topbbb'},$cgi->escapeHTML($row->{$cell}));
#		    print "<div style=\"float:left;padding-right:5px;\">" . $cgi->escapeHTML($row->{$cell}) . "</div>\n";
		    print "<div>" . $cgi->escapeHTML($row->{$cell}) . "</div>\n";
		}
	    }
