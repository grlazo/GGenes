...
# OK externaldb & database (ddbj,embl,genbank)
{
  ...
  if ($extdb->[0]->{'accession'})
  {
    ...
    $extdb->[1]->{'extdb'} = $cgi->a({href=>"http:///www.ebi.ac.uk/ena/data/view/".$extdb->[0]->{'accession'}." ",-target=>'_blank'},'Data at EMBL');
...
# OK externaldb (HarvEST:Barley BACs) External_DB[0], for BAC clones.
{
  my $sql = qq{
               select
                accession
               from sequenceexternaldb
               where sequenceid = $id
                and name = 'HarvEST:Barley BACs'
              };
  my $sth = $dbh->prepare($sql); $sth->execute;
  my $extdb = $sth->fetchall_arrayref({});
  if ($extdb->[0]->{'accession'})
  {
#      $extdb->[0]->{'extdb'} = $cgi->a({href=>"http://harvest-web.org/hweb/utilmenu.wc?job=RTRV_go&db=MOREX_HV3_10.4.1&bac=".$extdb->[0]->{'accession'},-target=>'_blank'},'BAC clone');
      $extdb->[0]->{'extdb'} = $cgi->a({href=>"http://harvest-web.org/hweb/utilmenu.wc?job=RTRV_go&db=MOREX_HV3_10.4.1&bac=".$extdb->[0]->{'accession'},-target=>'_blank'},"BAC clone $extdb->[0]->{'accession'}");
    delete($extdb->[0]->{'accession'});
    &print_element(
               $cgi,
               $dbh,
               'externaldb',
               'Data at harvest-web.org',
               $extdb,
               ['extdb_html'],
               []
               );
