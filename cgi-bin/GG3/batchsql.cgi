...
{ 
  ...
  {
    if ( $sql =~ /where.*in\s*$/ )	# 'names' there, and SQL query ends with "where...in" clause
    {
      &parsenames;
    ...
    else			# 'names' there, SQL query has "where...in," but does NOT end in "where...in"
    {
      ...
      if ( $sql =~ s/(where.*in\s*)\(.*\)\s*$/$1/is ) # get rid of everything after "where...in"
...
sub printform {
    ...
    print "<td valign=\"top\"><b>SQL query:</b>
