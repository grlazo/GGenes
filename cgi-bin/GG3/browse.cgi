...
#    print $cgi->p($cgi->b('No Results!'));
   if ($class){
	...
	if ($recordcount > $recordview) {	    
	    # begin table
	    print $cgi->start_p;
	    print $cgi->start_table({-border=>0,-style=>'width:330'});
...
#    print $cgi->start_p, $cgi->start_table({-border=>0,-cellpadding=>0,-cellspacing=>0,-style=>'font-size: small;'});
    print $cgi->start_p, $cgi->start_table({-border=>0,-cellpadding=>0,-cellspacing=>0,-class=>'main',-style=>'width:170;font-size:100%;'});
	...
	print $cgi->start_Tr;
        ...
        print $cgi->td('&nbsp;&nbsp;&nbsp;');
        print $cgi->td({-style=>'text-align:right'},$classcount->{'count'});
	print $cgi->end_Tr;
    }
...
sub print_form {
    ...
    print $cgi->start_table({-border=>0,-cellpadding=>10,-style=>'width:30%'});
    # form row
