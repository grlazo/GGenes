...
# http://wheat.pw.usda.gov/cgi-bin/graingenes/quickquery.cgi
...
and M1.id = M2.id
group by Locus, Chromosome, mapdata.name, maplocus.begin, mapdatalocus.scoringdata
...
qq{
-- Library, Treatment
--  may want to change to graingenes/search.cgi later?
select
...
where trait.name like '%'
group by Trait, QTL, mapdata.name, chromosomearm
order by Trait, QTL
},
...
group by QTL, Trait, mapdata.name, chromosomearm
order by Trait, QTL
},
...
select 
  allele.name as Allele,
  group_concat(germplasm.name) as Germplasm
from allele
