...
# dem 26mar08:  QTL report modified to include fields from Trait too.

# print qtl and trait report elements
...
##################### TRAIT
## TRAIT subreport
my $traitqtlid = $dbh->selectrow_array("select traitaffected_traitid from qtl where id = $id");
if ($traitqtlid)
{
    # name (trait)
    &print_element(
...
#  qtl  (trait)
	&print_element(
		   ...
		   select
                    ...
                    name as qtl_name
                   from qtl
                   where traitaffected_traitid = $traitqtlid
		   },
	       ['qtl_link'],
	       []
	       );   
...
&print_element(
               ...
               qq{
                   select remark
                   from traitremark
                   where traitid = $traitqtlid and type = 'Remark'
	       },
               ['remark'],
               []
               );
...
} # end TRAIT subreport (if $traitqtlid)
##################### TRAIT

    
##################### REFERENCE
## REFERENCE subreport
my $referenceqtlid = $dbh->selectrow_array("select referenceid from qtlreference where qtlid = $id");
if ($referenceqtlid)
{
    #name (reference)
    &print_element(
		   ...
		   qq{
			   ...
			   from reference
			   where reference.id = $referenceqtlid
		       },
                   ['reference_id'],
                   []
                   );
...
#  qtl  (reference)
        &print_element(
		       ...
		       qq{
                   ...
                   from qtl
		   join qtlreference on qtlreference.referenceid = $referenceqtlid
		   where qtlreference.qtlid = qtl.id
	       },
               ['qtl_link'],
               []
		       );
...
# end REFERENCE subreport (if $referenceqtlid)
##################### REFERENCE
...
&print_element(
		   ...
		   select
		    accession,
		    "Gramene" as description,
		    concat("http://www.gramene.org/db/ontology/search_term?id=",accession) as url,
		    remark
                   from traitontology
