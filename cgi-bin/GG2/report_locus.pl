...
# name & marker report link
{
    ...
    { 
      ...
      $locus->[0]->{'locus'} = $cgi->escapeHTML($locus->[0]->{'marker_name'}).'&nbsp;&nbsp;'.'[ '.$cgi->i($cgi->a({-href=>"$cgiurlpath/report.cgi?class=marker&name=".&geturlstring($locus->[0]->{'marker_name'}),-target=>'_blank'},'Marker Report')).' ]';
